...
const scopes = ['', 'ci', 'deps', 'docs', 'dx', 'release', 'style', 'tests'];

const packagesDir = path.join(__dirname, 'packages');
const packageNameDirs = fs.readdirSync(packagesDir, { withFileTypes: true })
    .filter(dirent => dirent.isDirectory())
    .map(dirent => dirent.name.toLocaleLowerCase());

const uniqueScope = [...scopes, ...packageNameDirs];
